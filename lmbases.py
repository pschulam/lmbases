...
class BSplines:
    def __init__(self, low, high, num_bases, degree,
        ...
        if use_quantiles_as_knots:
            knots = _get_quantile_knots(low, high, x, num_bases, degree)
        else:
            knots = _get_uniform_knots(low, high, num_bases, degree)

        if boundaries == 'stack':
            self._knots = _stack_pad_knots(knots, degree)
        elif boundaries == 'space':
            self._knots = _space_pad_knots(knots, degree)
...
def _stack_pad_knots(knots, degree):
    knots = list(knots)
    knots = ([knots[0]] * degree) + knots + ([knots[-1]] * degree)
    return knots

def _space_pad_knots(knots, degree):
    knots = list(knots)
    d1 = knots[1] - knots[0]
